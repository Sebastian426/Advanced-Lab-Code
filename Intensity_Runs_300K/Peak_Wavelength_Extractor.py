...
def wavelength_extraction(x, start_index, A=1.0095, alpha_A=0.0001,
                          B=9091.0570, alpha_B=0.0289):

    wavelength = (start_index - x / 10 - B) / A

    uncertainty = np.abs(
        (start_index - x / 10 - (B + alpha_B)) / (A + alpha_A) - wavelength
    )
...
def Peak_Wavelength(
    ...
    """
    Extract Lorentzian peak index and convert to wavelength.
    """

    # ---- Extract peak indices ----
    peak_indexes, peak_index_errors = peak_extractor(
        file=file,
        sheets=sheets,
        cols=cols,
    ...
    for peak in peak_indexes:
        wl, wl_err = wavelength_extraction(
            x=peak,
    ...
    return (
        ...
        np.atleast_1d(wavelength_errors),
        peak_indexes,
        peak_index_errors
    )
